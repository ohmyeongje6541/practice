...
async function rating() {
  ...
  const ratings = reviews.map((element) => {
    return element["rating"];
  });
  const sum = ratings.reduce((f, n) => {
    return f + n;
  });
...
async function tagList() {
  const response = await axios.get('https://dummyjson.com/posts/tag-list');
  const data = response["data"]
  console.log(data)
}
tagList();
...
async function posts() {
  const response = await axios.get('https://dummyjson.com/posts/tag/books')
  const data = response["data"]
  console.log(data)
}
posts();
...
async function total() {
  const response = await axios.get('https://dummyjson.com/products/search?q=glass')
  const data = response["data"]
