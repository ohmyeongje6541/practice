...
// 출력 결과는 배열 형식으로 출력한다
let completedfruits = fruits.map((fruit) => {
  return `맛있는 ${fruit}`
});
...
numbers1.reduce((accvalue, currElement, index) => {
...
// 새로운 배열을 출력한다
let completednumbers2 = numbers2.map((number2) => {
  return number2 * number2
});
console.log(completednumbers2)
/* 출력 결과
[1, 4, 9, 16, 25]
*/
...
// 새로운 배열을 출력한다
let completedname = names.map((name) => {
  return `${name}님`
});
...
// 새로운 배열을 출력한다
let completednumbers3 = numbers3.filter((number3) => {
  if (number3 % 2 === 0) {
    return true
  }
});
console.log(completednumbers3);
/* 출력 결과
[2, 4, 6, 8, 10]
*/
...
// 새로운 배열을 출력한다
let completedpeople = people1.filter((people) => {
  if (people["age"] >= 20) {
    return true
  }
});
console.log(completedpeople)
/* 출력 결과
...
// 합을 출력한다
let sum = numbers.reduce((acc, numbers) => {
  return acc + numbers;
});
console.log(sum)
/* 출력 결과
...
// 찾은 객체를 출력한다
let findpeople = people.find((people) => {
  if (people.id === 2) {
    return true
  }
});
...
let savedproducts1 = savedproducts.map((product) => {
  return `${product["name"]}, ${product["price"]}`
});
console.log(savedproducts1);
/* 출력 결과
[
  { name: "노트북", price: 1200000, category: "전자제품" },
  { name: "마우스", price: 25000, category: "전자제품" },
...
// 변환한 새로운 배열을 출력한다
let savedstudents = students.filter((student) => {
  if (student["score"] >= 70) {
    return true
  }
});
...
let savedstudents1 = savedstudents.map((student) => {
  return `${student["name"]} (${student["grade"]})`;
});
console.log(savedstudents1)
/* 출력 결과
