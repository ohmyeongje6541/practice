...
// 변수 grade에 75를 할당한다
// grade가 90 이상이면 "A등급", 80 이상이면 "B등급", 70 이상이면 "C등급", 그 외는 "D등급"을 출력한다
// 변수 grade를 출력한다
let grade = 75;
if (grade >= 90) {
  console.log("A등급");
...
// userAge가 20 이상이면서 isStudent이면 "성인 학생입니다", 아니면 "성인입니다"를 출력한다
...
// username이 "admin"이고 password가 "1234"이면 "로그인 성공", 아니면 "로그인 실패"를 출력한다
let username = "admin"
let password = "1234"
...
// userRole이 "admin" 또는 "editor"이면 "접근 권한이 있습니다", 아니면 "접근 권한이 없습니다"를 출력한다
let userRole = "editor"
...
// isLoggedIn이 false이면 "로그인이 필요합니다", 아니면 "이미 로그인되어 있습니다"를 출력한다
...
// userAge가 21 이상이고 hasLicense와 hasInsurance가 모두 true이면 "완벽한 운전자"를 출력한다
...
// isActive가 false이면 "비활성 계정"을 추가로 출력한다
let userType = "premium"
let userPoints = 2500
let isActive = true

if (userType && userPoints >= 2000) {
  console.log("VIP 등급")
} else if (userType && userPoints >= 1000) {
  console.log("프리미엄 등급")
} else if (userType = reqular && userPoints >= 1000) {
  console.log("일반 등급")
...
// TODO: 숫자 부호 판별
// 변수 number에 -5를 할당한다
// number가 0보다 크면 "양수", 0보다 작으면 "음수", 0이면 "0"을 출력한다
let number = -5;
if (number > 0) {
  console.log("양수")
} else if (number < 0) {
...
// memberLevel이 "gold" 또는 "platinum"이면 "VIP 혜택을 받을 수 있습니다", 아니면 "일반 혜택만 가능합니다"를 출력한다
let memberLevel = "gold"
if (memberLevel || memberLevel === "platinum") {
  console.log("VIP 해택을 받을 수 있습니다")
} else {
...
let userAge1 = 16
let isDecaf = false
if (drinkType && userAge1 >= 18) {
  console.log("커피 주문 가능")
} else if (userAge1 < 18 && isDecaf === true) {
  console.log("디카페인 가능")
