...
// 태그: 요소의 기능과 역할을 결정
// 속성: 요소에 추가적인 정보와 기능을 제공한다
// 컨텐츠: 텍스트 콘텐츠를 화면에 표시

// 문제 [개념]
// HTML 속성 중 class 속성의 역할에 대해 작성한다

// class 속성의 역할: 여러개의 요소를 그룹화 하기 위해 사용, 하나의 요소에 여러개의 클래스를 지정하거나, 하나의 클래스를 여러개의 요소에 지정할 수 있다

// 문제 [개념]
// JavaScript 원시 자료형 중 기본 자료형 3개에 대해 작성한다

// 문자열(String), 숫자형(Number), 불리언(Boolean)

// 문제 [개념]
// 변수 선언 키워드 let, const의 차이점을 작성한다

// let변수는 데이터 변환이 가능함(재할당 가능)
// const변수는 데이터 변환이 불가능함(재할당 불능)
...
// > : 두 개의 숫자나 문자길이가 있으면 그 중에 더 큰 걸 뜻 함
// < : 두 개의 숫자나 문자길이가 있으면 둘 중에 더 작은 거
// >= : 크거나 같다
// <= : 짝거나 같다
// === : 엄격하게 같은지 다른지 선별
// !== : 같지 않음을 의미
...
let isLogin = false;
let result2 = isLogin === true ? "로그인 상태입니다" : "로그아웃 상태입니다";
console.log(result2);
/* 출력 결과
로그아웃 상태입니다
*/
...
let accessResult = hasPermission === true ? "접근 허용" : "접근 거부";
...
// 배열의 생성: 대괄호[]를 사용해서 생성
// 배열 요소 접근 방법: 대괄호 []와 인덱스로 배열 원소에 접근
// 배열 요소 추가 방법: push(x) 통해서 배열의 끝에 데이터 추가
// 배열 요소 삭제 방법: pop() 배열의 마지막 원소를 제거하고, 제거한 원소 반환(return)
...
// 객체의 생성 방법: 중괄호{}를 사용해 생성
// 객체 속성 접근 방법: 마침표 . , 대괄호[]
// 객체 속성 추가 방법: 마침표 . 또는 대괄호 []를 사용해 속성 추가
...
// 함수 작성 후 아래 주석 처리된 테스트 코드의 주석을 해제하고 실행하여 출력 결과를 비교한다
// let student3 = { name: "홍길동", score: [85, 90, 94, 50] };

processScore = (student) => {
  let sum = student.score.reduce((a, b) => a + b);
  let average = sum / student.score.length;
  student["sum"] = sum;
...
processProduct = (product) => {
  product["discountPrice"] = product["price"] * (1 - 0.1);
  return product;
};
...
// map(): 콜백 함수의 반환값을 원소로 모아서 새로운 배열을 만듦
// filter(): 콜백 함수의 반환값이 true인 원소만 모아서 새로운 배열을 만든다

// 문제 [개념]
// 배열의 고차 메서드 map()과 filter()의 차이점을 작성한다

// map은 배열의 값을 변형하고 filter는 조건에 맞는 값만 선별
...
let newNumbers = numbers3.map((numbers3) => {
...
let taxIncludedPrices = prices.map((prices) => {
  return prices * (1 + 0.1);
});
...
let newNumbers2 = numbers4.filter((numbers4) => {
  return numbers4 % 2 === 0;
});
...
let highPriorityTodos = originTodos.filter((originTodos) => {
  return originTodos.priority === 0;
});
...
let mediumPriorityTodos = originTodos.filter((originTodos) => {
  return originTodos.priority === 1;
});
...
let incompleteTodos = originTodos.filter((originTodos) => {
  return originTodos.completed === false;
});
...
// 배열이나 객체의 내용을 펼쳐서 다른 배열이나 객체로 쉽게 복사하거나 합체하는 것
...
console.log(array1 === newarray); // false
...
let {brand : carBrand, model : carModel, year : carYear} = car2;
...
let [firstScore, secondScore, ...restScores] = scores2;
