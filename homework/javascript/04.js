...
/* 로직 코드 작성 */
highPriorityTodos = originTodos.filter((originTodo) => {
  if (originTodo["priority"] === HIGH_PRIORITY) {
    return true
  }
});
// 출력 코드
...
/* 로직 코드 작성 */
incompleteTodos = originTodos.filter((originTodo) => {
  if (originTodo["completed"] === false) {
    return true
  }
})
// 출력 코드
...
/* 로직 코드 작성 */
todosWithPriorityLabels = originTodos.map((originTodo) => {
